...
% Reset specific styles to make code blend into theorems
\lstset{
  language=C++,
...
\declaretheorem[style=thmbluebox,name=Code,numberwithin=section]{code}
...
We can view an object in c++ as a piece of memory that is used to store values. A object that is given a name is a variable. So
far we have seen programs that run instructions sequential in nature one after the other on define data types via objects and operations on this data
with functions that consisted with expressions and statements. So far previous sections have been doing what is called \textbf{procedural programming} where the main
focus is on creating procedures and in c++ called functions that build the logic. Given these functions we pass objects and perform operations on these 
objects and could return a value at the end back to the \textbf{caller}.
...
\begin{code}
  ...
  \begin{lstlisting}
    ...
    constexpr std::string_view aninalName(AnimalType type) {
      switch (type) {
        case cat: return "cat";
        case dog: return "dog";
...
We could perform the same operations using \textbf{OOP} in the following way.
