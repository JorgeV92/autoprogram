...
\begin{center}
\Large\textbf{Codeforces \,\decosix\, 1081 Div 2}%
\\
\LARGE\textbf{\decothreeleft~ Contest 2192 ~\decothreeright}%
\\[0.5ex]
...
\begin{enumerate}[(A)]
    \item 
    \begin{center}
        String rotating game.        
    \end{center}
    We are given a string of characters $s$ where it is defined that a \textbf{block} is a continguous substring of characters of the same \emph{type}.
    For example the string $aabcccdaa$ has the following blocks:
    \begin{itemize}
        \item $aa$ 
        \item $b$
        \item $ccc$
        ...
        \item $aa$
    \end{itemize}
    We are able to cyclically rotate the string however we want and then return the \emph{maximum} number of blocks in the string.

    \begin{Solution}
        We begin first to identify all blocks in the string and then consider possible rotations based on a match of the first character with the
        last charater $s[0] = s[n-1]$. If the these characters do not match and we have a block size greater than 2 then rotating to split the block will add $1$ more
        to the total block size in the end, otherwise the answer is just the total block size of the original string. The following algorithm solves this in $O(n)$ time, where
        $n$ is the length of the string.

        \begin{algo}
            \textul{\textsc{StringRotationGame}(\emph{s}):}\+
        \\  $\emph{n} \gets \emph{s}.\emph{length}$
        \\  $\emph{blocks} \gets 1$
        \\ for $i\gets 2$ to $n$\+
        \\      if $s[i-1] = s[i]$\+
        \\          $\emph{blocks} \gets \emph{blocks} + 1$\-\- 
        \\  if $\emph{blocks} \neq \emph{n}$ and $s[1] \neq s[n]$\+
        \\      $\emph{blocks} \gets \emph{blocks} + 1$\- 
        \\ return $\emph{blocks}$
        \end{algo}
